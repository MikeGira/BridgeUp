...
async function cacheFirst(request) {
  ...
  return (
    cached || (await fetchAndUpdate) || new Response("Offline", { status: 503 })
  );
}
...
async function networkFirst(request) {
  try {
    ...
    const shouldQueue =
      isPost && QUEUE_PATHS.some((p) => url.pathname.startsWith(p));
    ...
    if (shouldQueue) {
      ...
      return new Response(
        JSON.stringify({
          queued: true,
          message:
            "You are offline. Your submission has been saved and will be sent automatically when you reconnect.",
        }),
...
async function enqueueSubmission(request) {
  try {
    ...
    if (body.length > MAX_QUEUE_BODY_BYTES) {
      console.warn(
        "[SW] Submission body exceeds size limit (" +
          body.length +
          " bytes) — not queued for offline replay",
      );
      return;
    }
    ...
    if (currentDepth >= MAX_QUEUE_DEPTH) {
      console.warn(
        "[SW] Offline queue is full (" +
          currentDepth +
          " entries) — submission not queued",
      );
      return;
    }
  ...
  for (const { key, value } of entries) {
    try {
      ...
      if (response.ok) {
        ...
        console.warn(
          "[SW] Queued submission rejected by server (",
          response.status,
          ") — removing from queue:",
          value.url,
        );
      ...
      console.warn(
        "[SW] Still offline during sync replay for",
        value.url,
        "— will retry",
      );
    }
...
if ("PushManager" in self) {
  self.addEventListener("push", (event) => {
    try {
      ...
      event.waitUntil(self.registration.showNotification("BridgeUp", options));
    } catch (err) {
